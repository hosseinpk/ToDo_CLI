...
def rename_todo_list(old_name:str , new_name: str) -> bool:
    ...
    try:
        ...
        if not is_sqlite_file(old_todo):
            ...
            click.secho(f"Rename failed: '{old_name}' is not a valid SQLite file.",fg="yellow")
            return False
        ...
        click.secho(f"'{old_name}' todo list renamed to '{new_name}' successfully.", fg="green")
    ...
    # if (old_todo.exists()
    #     and old_todo.is_file()
    #     and is_sqlite_file(old_todo)):
        
    #     old_todo.rename(p.home() / ".todo" / f"{new_name}.db" )
